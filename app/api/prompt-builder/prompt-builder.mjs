...
export async function handlePromptBuilderRequest(request, configuredApiKey) {
  try {
...
async function readPromptRequest(request) {
  ...
  if (contentType.includes("application/json")) {
    ...
    value = typeof value === "string" ? value : value && typeof value === "object" ? value.request : undefined;
  } else {
...
async function generatePrompt(rawRequest, apiKey) {
  ...
  try {
    ...
    if (!prompt) throw new RouteError(502, "Prompt generation returned no usable text. Please try again.");
    return prompt;
  } catch (error) {
...
function openAiFailureMessage(status, payload) {
  ...
  const message = payload && typeof payload === "object" && typeof payload.error?.message === "string"
